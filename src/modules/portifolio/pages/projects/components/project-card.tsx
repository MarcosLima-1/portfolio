...
export function ProjectCard({ project, className, ...props }: ProjectCardProps) {
	return (
		<div
			className={cn(
				"group/project-card motion-preset-blink-fade motion-scale-out-102 w-87.5 cursor-pointer space-y-4 rounded-md hover:bg-accent/30",
				className,
			)}
			{...props}
		>
			<div className="relative aspect-video overflow-hidden rounded-md">
				<Image alt={project.title} priority height={100} width={100} src={project.images[0]} className="size-full object-cover" />
			...
			<div className="flex flex-col gap-4 p-2">
				<h3 className="font-bold text-lg text-primary max-md:text-sm">{project.title}</h3>
